"""Platform health-check registry — framework-free.
...
Relocated from ``core/health.py`` in Stage S1.4 (IP-001 §9 assigns the
monitoring concern to the shared layer); ``core.health`` re-exports for
compatibility. Checks may be synchronous or asynchronous; the registry
awaits awaitable results transparently.
"""
...
_logger = get_logger("atlas.shared.monitoring")
